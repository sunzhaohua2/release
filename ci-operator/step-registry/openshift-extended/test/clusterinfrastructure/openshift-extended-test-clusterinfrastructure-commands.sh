...
function run {
    test_scenarios=""
    echo "TEST_SCENARIOS_CLUSTERINFRASTRUCTURE: \"${TEST_SCENARIOS_CLUSTERINFRASTRUCTURE:-}\""
    echo "TEST_IMPORTANCE: \"${TEST_IMPORTANCE}\""
    echo "TEST_TIMEOUT_CLUSTERINFRASTRUCTURE: \"${TEST_TIMEOUT_CLUSTERINFRASTRUCTURE}\""
    if [[ -n "${TEST_SCENARIOS_CLUSTERINFRASTRUCTURE:-}" ]]; then
        readarray -t scenarios <<< "${TEST_SCENARIOS_CLUSTERINFRASTRUCTURE}"
    ...
    if [ "W${test_scenarios}W" == "WW" ]; then
        echo "fail to parse ${TEST_SCENARIOS_CLUSTERINFRASTRUCTURE}"
        exit 1
    fi
    ...
    ret_grep=0
    hack/ci-integration.sh  --dry-run | \
    ...
    echo "TEST_FILTERS_CLUSTERINFRASTRUCTURE: \"${hardcoded_filters};${TEST_FILTERS_CLUSTERINFRASTRUCTURE:-}\""
    test_filters="${hardcoded_filters};${TEST_FILTERS_CLUSTERINFRASTRUCTURE}"
    ...
    echo "MODULE_FILTERS_CLUSTERINFRASTRUCTURE: \"${MODULE_FILTERS_CLUSTERINFRASTRUCTURE:-}\""
    handle_module_filter "${MODULE_FILTERS_CLUSTERINFRASTRUCTURE}"
    ...
    -v \
    --timeout "${TEST_TIMEOUT_CLUSTERINFRASTRUCTURE}m" \
    --grace-period 5m \
    --fail-fast \
    --no-color \
    --focus-file "./case_selected" \
    --junit-report "${ARTIFACT_DIR}/junit" \
    --output-dir "${ARTIFACT_DIR}/extended.log" \
    "$@" \
    ./pkg/ -- --alsologtostderr -v 4  || ret_value=$?
...
function handle_result {
    resultfile=`ls -rt -1 ${ARTIFACT_DIR}/junit/junit_cluster_api_actuator_pkg_e2e_* 2>&1 || true`
    echo $resultfile
